...
- Added option to publish stubs for modules `php artisan vendor:publish --provider="Bitsnio\Modules\LaravelModulesServiceProvider" --tag="stubs"`
...
- Fixed bug: `Target class [Bitsnio\Modules\Commands\] does not exist.`
...
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Bitsnio\Modules\Module`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Bitsnio\Modules\Module`.
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Bitsnio\Modules\FileRepository`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Bitsnio\Modules\FileRepository`.
- **BREAKING** Removed the `__get` magic method on the `\Bitsnio\Modules\Module` class. Use `get()` or `json()->get()` instead.
...
- Laravel mix configuration (https://bitsnio.com/laravel-modules/v3/basic-usage/compiling-assets)
