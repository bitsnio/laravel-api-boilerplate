...
class BillingTimeRuleController extends Controller
{
     ...
     */
    public function update(UpdateBillingTimeRuleRequest $request, BillingTimeRule $billingTimeRule)
    {
        try 
        { 
            ...
            BillingTimeRule::where('id',$billing_time_rule_id->id)->update($billingTimeRule);
            unset($billingTimeRule['additional_service_id']);
            return Helper::successResponse(BaseResource::make($billingTimeRule));
        }
        catch (\Throwable $th) {
     ...
     */
    public function destroy(BillingTimeRule $billingTimeRule)
    {
        try 
        { 
